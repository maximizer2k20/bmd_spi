...
#define SPIS_INSTANCE 1 /**< SPIS instance index.   TSP changed SPIS_INSTANCE from 1 to 0   05092020 */
//static const nrf_drv_spis_t spis = NRF_DRV_SPIS_INSTANCE(SPIS_INSTANCE);/**< SPIS instance. */

static uint8_t       m_rx_buf[1000u];                       /**< RX buffer given a fixed length of 1024 bytes  */
...
void spis_event_handler(nrf_drv_spis_event_t event)
{
    if (event.evt_type == NRF_DRV_SPIS_XFER_DONE)
    {
        ...
        char temp = 0;
        int i;
        for (i = 0; i < sizeof(m_rx_buf)/sizeof(m_rx_buf[0]); ++i) {
        sum |= m_rx_buf[i];
            }
        if (sum != 0xff) {
        printf("At least one array element of m_rx_buf is -1\n");
          }
        strcat((char *)temp, (char *)m_rx_buf);
       
        NRF_LOG_INFO(" Transfer completed. Received:%s ",(uint32_t)m_rx_buf); // DGS
   
             NRF_LOG_INFO(" EOF"); // DGS
    }
}
...
{	
    ...
    sys_init();
    bmd_log("bmdwares %s\n", FIRMWARE_VERSION_STRING);  //TSP
//    if(sys_init_is_dtm()) // TSP
//    {
//        DTM_Init();
//    }
//    else
