...
#ifdef NRF52
    #define BLE_NUS_MAX_DATA_LEN        (GATT_EXTENDED_MTU_SIZE - 3)    /**< Maximum length of data (in bytes) that can be transmitted by the Nordic UART service module to the peer. */
#else
...
extern ble_uuid128_t   nus_base_uuid;
extern uint8_t         ble_nus_uuid_type;
...
typedef struct ble_nus_s //DGS 
{
    ...
    ble_gatts_char_handles_t baud_rate_handles;
    ble_gatts_char_handles_t parity_handles;
    ble_gatts_char_handles_t flow_control_handles;
    ble_gatts_char_handles_t stop_bits_handles;
    ...
    uint8_t                  stop_bits;
    uint8_t                  enable;
    uint8_t                  control[20];             /**< Beacon Configuration Control value EPS - Is this needed?? */
